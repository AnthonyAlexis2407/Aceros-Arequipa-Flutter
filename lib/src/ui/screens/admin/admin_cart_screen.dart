...
class AdminCartScreen extends StatefulWidget {
  const AdminCartScreen({super.key});

  @override
  State<AdminCartScreen> createState() => _AdminCartScreenState();
}

class _AdminCartScreenState extends State<AdminCartScreen> {
  ...
  // Helper to determine the plural unit of measure text (UNIDADES, PAQUETES, LARGOS)
  ...
  // Helper to get specification label (GRADO 60, ASTM A36)
  ...
  double _calculateItemWeightTons(CartItem item) {
    ...
    if (product.unitOfMeasure == 'UN' || product.category.toLowerCase().contains('plancha')) {
      ...
      if (product.id.contains('plancha')) {
        unitW = 81.44; // matching plate math: 81.44 * 25 = 2036 kg
      }
  ...
  // Pricing calculations matching screenshot
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              const Text(
                'La solicitud de despacho/recojo para el cliente ha sido registrada con éxito en Aceros Arequipa.',
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text(
              'El carrito de administrador está vacío',
              style: TextStyle(
                fontSize: 18,
                fontWeight: FontWeight.bold,
            ...
            const Text(
              'Explora el catálogo o añade materiales desde la sección de Inventario.',
            ...
            ElevatedButton.icon(
              onPressed: () {
                final shellState = context.findAncestorStateOfType<AdminShellState>();
                if (shellState != null) {
